...
# This script apparently looks at each sample replicate and computes the
# number of reads for that replicate, as well as the number of reads for
# all replicates. This is computed from the merged ELAND files, so it will
...
def main(stats_file, sample_config_file):
    ...
    for i, reads in enumerate(config.REPLICATES):
        rname = 'Rep'+str(i+1)
        replicates.append(Replicate(name=rname,
                                    ...
                                    read_files=reads))
    allname = 'RepAll'
    all_replicates = Replicate(name=allname,
