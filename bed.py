...
class PeakSeqBEDParser:
    ...
    def parse(self, line):
        fields = line.rstrip('\n').split('\t')
        if len(fields) == 8:
            return PeakSeqBED(chr=fields[0], start=fields[1], stop=fields[2],
                              sample_tags=fields[3], control_tags=fields[4],
                              enrichment=fields[5], excess=fields[6], p_value=float(fields[7]))
        elif len(fields) == 10:
            return PeakSeqBED(chr=fields[0], start=fields[1], stop=fields[2],
                              sample_tags=fields[3], control_tags=fields[4],
                              enrichment=fields[5], excess=fields[6], localmax=fields[7],
                              p_value=fields[8], q_value=fields[9])
        else:
...
class NarrowPeakBEDParser:
    ...
    def parse(self, line):
        ...
        return NarrowPeakBED(chr=fields[0], start=fields[1], stop=fields[2],
                             ...
                             signal_value=fields[6], p_value=fields[7], q_value=fields[8],
                             peak=fields[9])
